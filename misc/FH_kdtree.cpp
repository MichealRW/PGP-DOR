...
KDTree::KDTree(int dim_in, int max_sample_size)  // modified by FH
{
    sort_results = false;
    ...
    ind = new int [max_sample_size];
}

void KDTree::SetData(float *data_in)
{
    the_data = data_in;
    data = the_data;
}
...
KDTree::KDTree(float * data_in, int dim_in, int max_sample_size, bool _sort_results)  // modified by FH
{
    sort_results = _sort_results;
...
{
    FH_releaseMemory();
    num_of_nodes = 0;     // modified by FH for debugging reasons
    N  = num_in;
    for (int i=0; i<N; i++)
        ind[i] = i;
    root = build_tree_for_range(0, N-1, NULL);
//    std::cout<<std::endl<<"tree  "<<N<<"  "<<num_of_nodes<<std::endl;
}
...
void KDTree::spread_in_coordinate(int c, int l, int u, interval& interv) {
    ...
    smin = the_data[ind[l]*dim+c];
    smax = smin;
    
    // process two at a time.
    for (i=l+2; i<= u; i+=2) {
        lmin = the_data[ind[i-1]*dim+c];
        lmax = the_data[ind[i]  *dim+c];
    ...
    if (i == u+1) {
        float last = the_data[ind[u]*dim+c];
...
void KDTree::select_on_coordinate(int c, int k, int l, int u) {
    ...
    while (l < u) {
        ...
        for (int i=l+1; i<=u; i++) {
            if ( the_data[ ind[i] *dim+c] < the_data[t*dim+c]) {
                m++;
                swap(ind[i], ind[m]);
            }
        } // for i
...
int KDTree::select_on_coordinate_value(int c, float alpha, int l, int u) {
    ...
    while (lb < ub) {
        if (the_data[ind[lb]*dim+c] <= alpha) {
            lb++; // good where it is.
        } else {
            swap(ind[lb], ind[ub]);
            ub--;
    ...
    if(lb==u || lb==l)       // modified by FH. to avoid the case where all the values are the same : the_data[ind[lb]*dim+c] == alpha
        return(l+int((u-l)/2));
...
void KDTree::n_nearest_brute_force(std::vector<float>& qv, int nn, KDTreeResultVector& result) {
    ...
    for (int i=0; i<N; i++) {
        ...
        KDTreeResult e;
        for (int j=0; j<dim; j++) {
            dis += squared( the_data[i*dim+j] - qv[j]);
        }
        e.dis = dis;
        e.idx = i;
        result.push_back(e);
...
// constructor
KDTreeNode::KDTreeNode(int _dim) : box(_dim)
{
    dim = _dim;
    left = right = NULL;
    //
